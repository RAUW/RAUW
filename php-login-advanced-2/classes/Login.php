...
class Login
{
    ...
    public function __construct()
    {
        ...
        // if user has an active session on the server
            ...
            if (isset($_POST["user_edit_submit_name"])) {
                ...
                $this->editUserName($_POST['user_email']);
    ...
    private function getUserData($user_email)
    {
        ...
        if ($this->databaseConnection()) {
            ...
            $query_user = $this->db_connection->prepare('SELECT * FROM gebruikers WHERE user_email = :user_email');
        ...
        if (isset($_COOKIE['rememberme'])) {
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                if ($this->databaseConnection()) {
                    ...
                    $sth = $this->db_connection->prepare("SELECT user_id, user_email FROM gebruikers WHERE user_id = :user_id
    ...
    private function loginWithPostData($user_email, $user_password, $user_rememberme)
    {
        ...
        // if POST data (from login form) contains non-empty user_email and non-empty user_password
            ...
            // using PHP 5.5's password_verify() function to check if the provided passwords fits to the hash of that user's password
                ...
                $sth = $this->db_connection->prepare('UPDATE gebruikers '
            ...
            // has the user activated their account with the verification email
                ...
                if (defined('HASH_COST_FACTOR')) {
                    ...
                    if (password_needs_rehash($result_row->user_password_hash, PASSWORD_DEFAULT, array('cost' => HASH_COST_FACTOR))) {
                        ...
                        $query_update = $this->db_connection->prepare('UPDATE gebruikers SET user_password_hash = :user_password_hash WHERE user_id = :user_id');
    ...
    private function newRememberMeCookie()
    {
        ...
        if ($this->databaseConnection()) {
            ...
            $sth = $this->db_connection->prepare("UPDATE gebruikers SET user_rememberme_token = :user_rememberme_token WHERE user_id = :user_id");
    ...
    private function deleteRememberMeCookie()
    {
        ...
        if ($this->databaseConnection()) {
            ...
            $sth = $this->db_connection->prepare("UPDATE gebruikers SET user_rememberme_token = NULL WHERE user_id = :user_id");
    ...
    public function editUserEmail($user_email)
    {
        ...
        // user mail cannot be empty and must be in email format
            ...
            if (isset($result_row->user_id)) {
                ...
                $query_edit_user_email = $this->db_connection->prepare('UPDATE gebruikers SET user_email = :user_email WHERE user_id = :user_id');
    ...
    public function setPasswordResetDatabaseTokenAndSendMail($user_email)
    {
        ...
        if (empty($user_email)) {
            ...
            if (isset($result_row->user_id)) {
                ...
                // database query:
                $query_update = $this->db_connection->prepare('UPDATE gebruikers SET user_password_reset_hash = :user_password_reset_hash,
                ...
                if ($query_update->rowCount() == 1) {
                    ...
                    $this->sendPasswordResetMail($result_row->user_email, $user_password_reset_hash);
                    return true;
                } else {
    ...
    /** 2014-07-02: hier gaat het fout bij password reset!!
    ...
    public function editNewPassword($user_email, $user_password_reset_hash, $user_password_new, $user_password_repeat)
    {
        ...
        // if database connection opened
            ...
            // write users new hash into database
            $query_update = $this->db_connection->prepare('UPDATE gebruikers SET user_password_hash = :user_password_hash,
